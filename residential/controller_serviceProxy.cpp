...
#include "controller_serviceProxy.h"
...
// proxy allocator
class controller_serviceProxyAllocator: public staff::IProxyAllocator
{
public:
  controller_serviceProxyAllocator()
  {
    try
    {
      staff::ServiceFactory::Inst().RegisterProxyAllocator(typeid(controller_service).name(), *this);
    }
    STAFF_CATCH_ALL_DESCR("Failed to register proxy allocator controller_service");
  }
  ...
  {
    std::auto_ptr<controller_serviceProxy> tpProxy(new controller_serviceProxy);
  ...
  {
    std::auto_ptr<controller_serviceProxy> tpProxy(new controller_serviceProxy);
    tpProxy->Init(sBaseUri + (sServiceName.empty() ? "energy.Controller.controller_service" : sServiceName),
                  sSessionId, sInstanceId);
    return tpProxy.release();
  }
};

controller_serviceProxyAllocator tcontroller_serviceProxyAllocatorInitializer;

// asynch proxies


// service proxy
controller_serviceProxy::controller_serviceProxy()
{
}
...
void controller_serviceProxy::Init(const std::string& sServiceUri, const std::string& sSessionId, const std::string& sInstanceId)
{
  staff::IService::Init("energy.Controller.controller_service", sSessionId, sInstanceId);
  m_sServiceUri = !sServiceUri.empty() ? sServiceUri : "http://10.96.12.47:9090/axis2/services/energy.Controller.controller_service";
  m_tClient.Init(m_sServiceUri);
  staff::Options& rOptions = m_tClient.GetOptions();
  rOptions.SetDefaultNamespace("http://tempui.org/energy.Controller", "");
...
void controller_serviceProxy::Deinit()
{
...
staff::ServiceClient* controller_serviceProxy::GetClient()
{
  return &m_tClient;
}


double controller_serviceProxy::COpostsync(int control_mode, double base_setpoint)
{
  staff::Operation tOperation("COpostsync", "COpostsyncResult");
  staff::DataObject& rdoRequest = tOperation.Request();
  rdoRequest.SetNamespaceUriGenPrefix("http://tempui.org/energy.Controller");
...
double controller_serviceProxy::COsync(double load, double demand, int pstate, double total, double target, double pAvg, double pStd, double base_setpoint, int control_mode, double market_current_frameclearing_price)
{
...
double controller_serviceProxy::COinit(double market_init_price, int pstate, int market_period, int control_mode, double psetpoint, double base_setpoint, double heating_base_setpoint, double cooling_base_setpoint)
{
...
int controller_serviceProxy::COpresync(double base_setpoint, double psetpoint, int control_mode)
{
