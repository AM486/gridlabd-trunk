...
#include "microturbine_serviceProxy.h"
...
// proxy allocator
class microturbine_serviceProxyAllocator: public staff::IProxyAllocator
{
public:
  microturbine_serviceProxyAllocator()
  {
    try
    {
      staff::ServiceFactory::Inst().RegisterProxyAllocator(typeid(microturbine_service).name(), *this);
    }
    STAFF_CATCH_ALL_DESCR("Failed to register proxy allocator microturbine_service");
  }
  ...
  {
    std::auto_ptr<microturbine_serviceProxy> tpProxy(new microturbine_serviceProxy);
  ...
  {
    std::auto_ptr<microturbine_serviceProxy> tpProxy(new microturbine_serviceProxy);
    tpProxy->Init(sBaseUri + (sServiceName.empty() ? "energy.Microturbine.microturbine_service" : sServiceName),
                  sSessionId, sInstanceId);
    return tpProxy.release();
  }
};

microturbine_serviceProxyAllocator tmicroturbine_serviceProxyAllocatorInitializer;

// asynch proxies


// service proxy
microturbine_serviceProxy::microturbine_serviceProxy()
{
}
...
void microturbine_serviceProxy::Init(const std::string& sServiceUri, const std::string& sSessionId, const std::string& sInstanceId)
{
  staff::IService::Init("energy.Microturbine.microturbine_service", sSessionId, sInstanceId);
  m_sServiceUri = !sServiceUri.empty() ? sServiceUri : "http://10.96.12.47:9090/axis2/services/energy.Microturbine.microturbine_service";
  m_tClient.Init(m_sServiceUri);
  staff::Options& rOptions = m_tClient.GetOptions();
  rOptions.SetDefaultNamespace("http://tempui.org/energy.Microturbine", "");
...
void microturbine_serviceProxy::Deinit()
{
...
staff::ServiceClient* microturbine_serviceProxy::GetClient()
{
  return &m_tClient;
}


double microturbine_serviceProxy::microturbine_init()
{
  staff::Operation tOperation("microturbine_init", "microturbine_initResult");
  staff::DataObject& rdoRequest = tOperation.Request();
  rdoRequest.SetNamespaceUriGenPrefix("http://tempui.org/energy.Microturbine");
  // synchronous call
...
int microturbine_serviceProxy::microturbine_presync(int t0, int t1)
{
