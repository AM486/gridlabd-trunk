...
int solar_service::create(void) 
{
	...
	price= psolarGen_service->solarcreate();
...
int solar_service::init(OBJECT *parent)
{
	...
	climate_result = psolarGen_service->solarinit(gen_mode_v, gen_status_v, panel_type_v);
...
TIMESTAMP solar_service::sync(TIMESTAMP t0, TIMESTAMP t1) 
{
	...
	VA_Out = psolarGen_service->solarsync(t0, t1);
//	 *pLine_I =I_Out;
  //   *pCircuit_V =V_Out;
