/** $Id: controller_client.cpp 4738 2014-07-03 00:55:39Z dchassin $
	Copyright (C) 2009 Battelle Memorial Institute
	@file auction.cpp
	@defgroup controller_client Transactive controller_client, OlyPen experiment style
	@ingroup market
...
CLASS* controller_client::oclass = NULL;

controller_client::controller_client(MODULE *module){
	if (oclass==NULL)
	{
		oclass = gl_register_class(module,"controller_client",sizeof(controller_client),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_AUTOLOCK);
		if (oclass==NULL)
			throw "unable to register class controller_client";
		else
		...
		if (gl_publish_variable(oclass,
			...
			// new stuff
			PT_double, "market_price", PADDR(clear_price), PT_DESCRIPTION, "the current market clearing price seen by the controller_client.",
		...
		memset(this,0,sizeof(controller_client));
	}
}

int controller_client::create(){
	
	return 1;
}

/** provides some easy default inputs for the transactive controller_client,
	 and some examples of what various configurations would look like.
 **/
void controller_client::cheat(int simplemode){
	switch(simplemode){
		case SM_NONE:
			break;
		case SM_HOUSE_HEAT:
 ...
 **/
void controller_client::fetch(double **prop, char *name, OBJECT *parent){
	...
	*prop = gl_get_double_by_name(parent, name);
	if(*prop == NULL){
		...
		char msg[256];
		sprintf(tname, "controller_client:%i", hdr->id);
		if(*name == NULL)
			sprintf(msg, "%s: controller_client unable to find property: name is NULL", namestr);
		else
			sprintf(msg, "%s: controller_client unable to find %s", namestr, name);
		throw(msg);
	}
}
 ...
 **/
int controller_client::init(OBJECT *parent){
...
//	double high, low;
	...
	if(parent == NULL){
		gl_error("%s: controller_client has no parent, therefore nothing to control", namestr);
		return 0;
	}

	if(pMarket == NULL){
		gl_error("%s: controller_client has no market, therefore no price signals", namestr);
		return 0;
	}
	...
	if(gl_object_isa(pMarket, "auction")){
		...
		gl_error("controller_clients only work when attached to an 'auction' object");
		return 0;
	}
	...
	if(dPeriod == 0.0){
		if((pMarket->flags & OF_INIT) != OF_INIT){
			char objname[256];
			gl_verbose("controller_client::init(): deferring initialization on %s", gl_name(pMarket, objname, 255));
			return 2; // defer
		}
	...
	if(bid_delay > period){
		gl_warning("Bid delay is greater than the controller_client period. Resetting bid delay to 0.");
		bid_delay = 0;
	}

	if(target[0] == 0){
		GL_THROW("controller_client: %i, target property not specified", hdr->id);
	}
	if(setpoint[0] == 0 && control_mode == CN_RAMP){
		GL_THROW("controller_client: %i, setpoint property not specified", hdr->id);;
	}
	if(demand[0] == 0 && control_mode == CN_RAMP){
		GL_THROW("controller_client: %i, demand property not specified", hdr->id);
	}
	if(deadband[0] == 0 && use_predictive_bidding == TRUE && control_mode == CN_RAMP){
		GL_THROW("controller_client: %i, deadband property not specified", hdr->id);
	}
	if(total[0] == 0){
		GL_THROW("controller_client: %i, total property not specified", hdr->id);
	}
	if(load[0] == 0){
		GL_THROW("controller_client: %i, load property not specified", hdr->id);
	}

	if(heating_setpoint[0] == 0 && control_mode == CN_DOUBLE_RAMP){
		GL_THROW("controller_client: %i, heating_setpoint property not specified", hdr->id);;
	}
	if(heating_demand[0] == 0 && control_mode == CN_DOUBLE_RAMP){
		GL_THROW("controller_client: %i, heating_demand property not specified", hdr->id);
	}

	if(cooling_setpoint[0] == 0 && control_mode == CN_DOUBLE_RAMP){
		GL_THROW("controller_client: %i, cooling_setpoint property not specified", hdr->id);;
	}
	if(cooling_demand[0] == 0 && control_mode == CN_DOUBLE_RAMP){
		GL_THROW("controller_client: %i, cooling_demand property not specified", hdr->id);
	}
...
fetch(&pAvg, avg_target.get_string(), pMarket);
	...
	//return value on last_p
	 ...
	 int COinitResult = pcontroller_service->COinit(market_init_price, pstate, market_period, control_mode, psetpoint, base_setpoint, heating_base_setpoint, cooling_base_setpoint);
...
int controller_client::isa(char *classname)
{
	return strcmp(classname,"controller_client")==0;
}


TIMESTAMP controller_client::presync(TIMESTAMP t0, TIMESTAMP t1){
...
TIMESTAMP controller_client::sync(TIMESTAMP t0, TIMESTAMP t1){
...
double bid = -1.0;
	...
	double COsyncResult = pcontroller_service->COsync(load, demand2, pstate, total, target, pAvg2, pStd2, base_setpoint, control_mode, market_current_frameclearing_price);
...
TIMESTAMP controller_client::postsync(TIMESTAMP t0, TIMESTAMP t1){
...
EXPORT int create_controller_client(OBJECT **obj, OBJECT *parent)
{
	try
	{
		*obj = gl_create_object(controller_client::oclass);
		if (*obj!=NULL)
		{
			controller_client *my = OBJECTDATA(*obj,controller_client);
	...
	CREATE_CATCHALL(controller_client);
}

EXPORT int init_controller_client(OBJECT *obj, OBJECT *parent)
{
	try
	{
		if (obj!=NULL)
		{
			return OBJECTDATA(obj,controller_client)->init(parent);
		}
		else
			return 0;
	}
	INIT_CATCHALL(controller_client);
}

EXPORT int isa_controller_client(OBJECT *obj, char *classname)
{
	if(obj != 0 && classname != 0){
		return OBJECTDATA(obj,controller_client)->isa(classname);
	} else {
		return 0;
	}
}

EXPORT TIMESTAMP sync_controller_client(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
	...
	SYNC_CATCHALL(controller_client);
}
