...
#include "wheater_serviceProxy.h"
...
// proxy allocator
class wheater_serviceProxyAllocator: public staff::IProxyAllocator
{
public:
  wheater_serviceProxyAllocator()
  {
    try
    {
      staff::ServiceFactory::Inst().RegisterProxyAllocator(typeid(wheater_service).name(), *this);
    }
    STAFF_CATCH_ALL_DESCR("Failed to register proxy allocator wheater_service");
  }
  ...
  {
    std::auto_ptr<wheater_serviceProxy> tpProxy(new wheater_serviceProxy);
  ...
  {
    std::auto_ptr<wheater_serviceProxy> tpProxy(new wheater_serviceProxy);
    tpProxy->Init(sBaseUri + (sServiceName.empty() ? "energy.WHeater.wheater_service" : sServiceName),
                  sSessionId, sInstanceId);
    return tpProxy.release();
  }
};

wheater_serviceProxyAllocator twheater_serviceProxyAllocatorInitializer;

// asynch proxies


// service proxy
wheater_serviceProxy::wheater_serviceProxy()
{
}
...
void wheater_serviceProxy::Init(const std::string& sServiceUri, const std::string& sSessionId, const std::string& sInstanceId)
{
  staff::IService::Init("energy.WHeater.wheater_service", sSessionId, sInstanceId);
  m_sServiceUri = !sServiceUri.empty() ? sServiceUri : "http://10.96.12.47:9090/axis2/services/energy.WHeater.wheater_service";
  m_tClient.Init(m_sServiceUri);
  staff::Options& rOptions = m_tClient.GetOptions();
  rOptions.SetDefaultNamespace("http://tempui.org/energy.WHeater", "");
...
void wheater_serviceProxy::Deinit()
{
...
staff::ServiceClient* wheater_serviceProxy::GetClient()
{
  return &m_tClient;
}


int wheater_serviceProxy::WHcreate()
{
  staff::Operation tOperation("WHcreate", "WHcreateResult");
  staff::DataObject& rdoRequest = tOperation.Request();
  rdoRequest.SetNamespaceUriGenPrefix("http://tempui.org/energy.WHeater");
  // synchronous call
...
int wheater_serviceProxy::WHinit()
{
...
int wheater_serviceProxy::WHsync(int t0, int t1, double heating_element_capacity, double temperature, double tank_setpoint, double tank_UA, double water_demand, double tank_volume, double thermostat_deadband)
{
...
int wheater_serviceProxy::WHcommit()
{
...
int wheater_serviceProxy::WHtank_state()
{
...
int wheater_serviceProxy::WHpostsync()
{
...
void wheater_serviceProxy::WHset_time_to_transition()
{
...
int wheater_serviceProxy::WHset_current_model_and_load_state()
{
...
double wheater_serviceProxy::WHdhdt(double h)
{
...
double wheater_serviceProxy::WHactual_kW()
{
...
double wheater_serviceProxy::WHnew_time_1node(double T0, double T1)
{
...
double wheater_serviceProxy::WHnew_temp_1node(double T0, double delta_t)
{
...
double wheater_serviceProxy::WHnew_time_2zone(double h0, double h1)
{
...
double wheater_serviceProxy::WHnew_h_2zone(double h0, double delta_t)
{
...
double wheater_serviceProxy::WHget_Tambient()
{
