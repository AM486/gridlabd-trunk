...
std::auto_ptr< ::energy::WHeater::wheater_service > pwheater_service(::staff::ServiceFactory::Inst().GetService< ::energy::WHeater::wheater_service >());
...
int WH_as_service::create()
{
	...
	int WHcreateResult = pwheater_service->WHcreate();
	return SUCCESS;
}

int WH_as_service::init(OBJECT *parent)
{
	int WHinitResult = pwheater_service->WHinit();
	a=9;
	return SUCCESS;
}
...
//	int WHsyncResult = pwheater_service->WHsync(0, 1, heating_element_capacity, temperature, tank_setpoint, tank_UA, water_demand, tank_volume, thermostat_deadband);
	int WHsyncResult = pwheater_service->WHsync(t0, t1, 4, 110, 120, 3.3, 0.7, 28, 4);
	printf("%f",WHsyncResult);
	return TS_NEVER;
}
