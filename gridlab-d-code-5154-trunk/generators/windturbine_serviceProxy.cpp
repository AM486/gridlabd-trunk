...
public:
  ...
  {
    std::auto_ptr<windturbine_serviceProxy> tpProxy(new windturbine_serviceProxy);
    tpProxy->Init(sBaseUri + (sServiceName.empty() ? "windturbine_service" : sServiceName),
...
{
  staff::IService::Init("windturbine_service", sSessionId, sInstanceId);
  m_sServiceUri = !sServiceUri.empty() ? sServiceUri : "http://10.96.12.47:9090/axis2/services/windturbine_service";
