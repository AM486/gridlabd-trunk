...
#include "controllerserviceProxy.h"
...
// proxy allocator
class controllerserviceProxyAllocator: public staff::IProxyAllocator
{
public:
  controllerserviceProxyAllocator()
  {
    try
    {
      staff::ServiceFactory::Inst().RegisterProxyAllocator(typeid(controllerservice).name(), *this);
    }
    STAFF_CATCH_ALL_DESCR("Failed to register proxy allocator controllerservice");
  }
  ...
  {
    std::auto_ptr<controllerserviceProxy> tpProxy(new controllerserviceProxy);
  ...
  {
    std::auto_ptr<controllerserviceProxy> tpProxy(new controllerserviceProxy);
    tpProxy->Init(sBaseUri + (sServiceName.empty() ? "controllerservice" : sServiceName),
                  sSessionId, sInstanceId);
    return tpProxy.release();
  }
};

controllerserviceProxyAllocator tcontrollerserviceProxyAllocatorInitializer;

// asynch proxies


// service proxy
controllerserviceProxy::controllerserviceProxy()
{
}
...
void controllerserviceProxy::Init(const std::string& sServiceUri, const std::string& sSessionId, const std::string& sInstanceId)
{
  staff::IService::Init("controllerservice", sSessionId, sInstanceId);
  m_sServiceUri = !sServiceUri.empty() ? sServiceUri : "http://10.96.12.47:9090/axis2/services/controllerservice";
...
void controllerserviceProxy::Deinit()
{
...
staff::ServiceClient* controllerserviceProxy::GetClient()
{
  return &m_tClient;
}


int controllerserviceProxy::COinit(double market_init_price, int pstate, int market_period, int control_mode, double psetpoint, double base_setpoint, double heating_base_setpoint, double cooling_base_setpoint)
{
...
int controllerserviceProxy::COpresync(double base_setpoint, double psetpoint, int control_mode)
{
...
double controllerserviceProxy::COsync(double load, double demand, int pstate, double total, double target, double pAvg, double pStd, double base_setpoint, int control_mode, double market_current_frameclearing_price)
{
...
double controllerserviceProxy::COpostsync(int control_mode, double base_setpoint)
{
